...
-- TODO: Remove gate logic once they cant be hit without making them visible. There are several checks blocking it at the moment and it would break some abilities.
-----------------------------------
...
content.hideGates = function(mob)
    local instance = mob:getInstance()

    for gateId = ID.mob.DILAPIDATED_GATE, ID.mob.DILAPIDATED_GATE + 2 do
        GetMobByID(gateId, instance):setUntargetable(true)
    end
end
