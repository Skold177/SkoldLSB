...
-- TODO: Work with Xaver to send this logic upstream ASAP after launch.
-----------------------------------
...
local function handleItemRoll(player, zoneId, rank, currentHour, currentWeather, elementalOreActive)
    ...
    -- Regular Item
    return loot[1].itemId, loot[1].rank
end
...
-- Caches are important to keep since the majority of our item weights are captured from the retail digging system.
-- Caches normally award 100-700 points in 5 point increments. This amount is not appropriate for the 75 era so we will award 20-140 in 5 point increments instead. (80% reduction)
...
m:addOverride('xi.chocoboDig.start', function(player)
    ...
    -- If the player has already obtained 100 items for the day, play the digging animation and give them nothing.
    if itemsDug >= 100 then
    ...
    local itemId, itemRank = handleItemRoll(player, zoneId, skillRank, currentHour, currentWeather, elementalOreActive)
    ...
    if itemId == 0 then
        ...
        handleItemObtained(player, text, itemId, itemRank)
    end
